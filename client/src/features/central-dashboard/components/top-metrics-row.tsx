...
export function TopMetricsRow() {
  ...
  if (metrics.length > 0) {
    // 0: Total DB Size
    metrics[0] = { ...metrics[0], value: totalDbSizeValue as unknown as string };
    // 1: Total digitization/migration
    metrics[1] = { ...metrics[1], value: getEnrollValue("migration") as unknown as string };
    // 2: Total Slip Capture
    metrics[2] = { ...metrics[2], value: getEnrollValue("tp_enroll") as unknown as string };
    // 3: Total Live Enrollment
    metrics[3] = { ...metrics[3], value: getEnrollValue("live_enroll") as unknown as string };
    // 4: Total Chance Print
    metrics[4] = { ...metrics[4], value: getEnrollValue("lt_enroll") as unknown as string };
  }
