...
export function FilterBar() {
  ...
  if (masterData?.data) {
    ...
    Object.keys(dataMap).sort().forEach((state) => {
      stateOptions.push({ label: state, value: state });
    });

    const isAllStatesSelected = selectedStates.length === 0 || selectedStates.includes("all");
    let districtsToShow = new Set<string>();
    ...
    Array.from(districtsToShow).sort().forEach(d => {
    ...
    <div className="flex items-center flex-wrap gap-3 p-4 bg-slate-50 border-b border-slate-200 w-full">
      ...
      <div className="flex items-center gap-3 shrink-0 z-50">
        ...
        <div className="flex items-center gap-2">
          ...
          <Input className="w-48 bg-white shadow-sm" placeholder="Police Station..." />
        </div>
      ...
      <div className="flex items-center gap-3 shrink-0">
        <div className="flex items-center gap-2 relative">
          ...
          <Input 
            type="date" 
            className="w-[150px] bg-white shadow-sm text-slate-600" 
